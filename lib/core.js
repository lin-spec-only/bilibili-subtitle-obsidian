...
export function normalizeVideoInput(rawInput) {
  ...
  if (
    ...
    /^\/list\/watchlater\/?$/i.test(url.pathname)
  ) {
    const watchLaterBvid = String(url.searchParams.get("bvid") || "").trim();
    if (/^BV[0-9A-Za-z]+$/.test(watchLaterBvid)) {
      const videoUrl = new URL(`/video/${watchLaterBvid}`, "https://www.bilibili.com");
...
export function selectPreferredSubtitleTrackIndex(tracks) {
  if (!Array.isArray(tracks) || !tracks.length) {
    return 0;
  }

  const isChinese = (track) => {
    const language = `${track?.language || ""} ${track?.languageLabel || ""}`.toLowerCase();
    return /\bzh(?:-|_|$)|chinese|中文|汉语|國語|国语/.test(language);
  };
  const firstIndex = (predicate) => tracks.findIndex(predicate);

  // Prefer an exportable Chinese track, but keep a usable fallback for other videos.
  return Math.max(
    0,
    firstIndex((track) => isChinese(track) && hasUsableSubtitleContent(track)),
    firstIndex(hasUsableSubtitleContent),
    firstIndex(isChinese)
  );
}
